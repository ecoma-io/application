# **Tài liệu Thiết kế Miền: Bounded Context Identity & Access Management (IAM)**
...
- **Session (Entity độc lập, stateful):** Đại diện cho một phiên làm việc đã được xác thực của người dùng. Là Entity độc lập để dễ dàng quản lý vòng đời (tạo, hủy, tìm kiếm theo User). **Sử dụng Session Token stateful.**
  - **ID:** Unique identifier (UUID).
...
- **AuthenticationService:**
  - **Trách nhiệm:** Xử lý quy trình đăng nhập, xác minh thông tin xác thực, tạo và quản lý Token/Session, có tính đến ngữ cảnh phạm vi (nội bộ/tổ chức). **Sử dụng Session Token stateful.**
