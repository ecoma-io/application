...
- **AuditLogEntry:** Là Aggregate Root đại diện cho một bản ghi kiểm tra duy nhất về một hành động cụ thể trong hệ thống. Đây là đơn vị dữ liệu cốt lõi mà ALM quản lý.
  ...
  - _Behavior:_ Persist() \- Hành vi chính là được tạo ra và lưu trữ. Sau khi lưu trữ thành công, bản ghi log được coi là bất biến. Không có hành vi Update hoặc Delete trên Aggregate Root này thông qua Domain Service.
...
- **Tương tác với Core BCs và Feature BCs (Nguồn phát sinh Log):**
  ...
  - **Cơ chế:** Các BC phát sinh log không gọi trực tiếp ALM. Thay vào đó, họ phát ra các Domain Event thông qua Message Broker. **ALM hoạt động như một Consumer của các Event này.**
  - **ALM lắng nghe Event:** Application Service của ALM (AuditLogIngestionApplicationService) đăng ký lắng nghe các loại Domain Event quan trọng từ Message Broker.
...
- **ALM-UC-8.1.1: Thu thập và Lưu trữ Audit Log Entry**
  ...
  - **Mô tả:** ALM lắng nghe các Domain Event được phát ra bởi các BC khác. Khi nhận được Event cần audit, ALM Ingestion Worker (là một Background Task) sẽ xử lý Event payload, tạo và lưu trữ một bản ghi AuditLogEntry mới. Đây là một Command vì nó thay đổi trạng thái của hệ thống bằng cách thêm dữ liệu mới.
  - **Service liên quan:** Lắng nghe Domain Event từ Message Broker. Sử dụng AuditLogIngestionApplicationService để xử lý Event. Sử dụng AuditLogService để tạo và lưu trữ AuditLogEntry. Sử dụng Repository để tương tác với kho dữ liệu.
...
- **AuditLogService:**
  ...
  - **Các phương thức tiềm năng:** PersistAuditLogEntry(...), FindAuditLogs(criteria: AuditLogQueryCriteria), ApplyRetentionPolicy(policy: RetentionPolicy).
...
- **AuditLogIngestionApplicationService:**
  - **Trách nhiệm:** Lắng nghe các Domain Event từ các BC khác (ví dụ: thông qua Message Broker). Trích xuất dữ liệu cần thiết từ Event payload và gọi AuditLogService.PersistAuditLogEntry(). Đảm bảo việc xử lý Event là đáng tin cậy (ví dụ: sử dụng cơ chế Exactly-Once Processing nếu Message Broker hỗ trợ).
  - **Các phương thức tiềm năng:** Handle\[EventName\](event).
...
- **\[Tên Event từ BC khác\]** (Ví dụ: UserLoggedInEvent từ IAM, OrderCreatedEvent từ ODM, SubscriptionActivated từ BUM)
  - Phát ra khi một hành động nghiệp vụ quan trọng xảy ra trong một BC nguồn.
  - **Mục đích xử lý:** ALM lắng nghe event này để trích xuất thông tin và tạo một bản ghi AuditLogEntry.
  - **Payload dự kiến:** (Tùy thuộc vào từng loại Event cụ thể từ BC nguồn. ALM sẽ cần các thông tin như Timestamp, Initiator, BoundedContext, ActionType, EntityId, EntityType, TenantId, ContextData, Status, FailureReason từ payload của Event nguồn).
...
Tài liệu này đã được cập nhật để tuân thủ cấu trúc và mức độ chi tiết của các tài liệu IAM và BUM, bao gồm việc bổ sung phần Các Khía cạnh Quan trọng của Miền, tổ chức lại luồng nghiệp vụ thành Use Cases và chi tiết hóa các Domain Event (Published/Consumed, payload dự kiến). Đây là nền tảng để chuyển sang giai đoạn thiết kế kỹ thuật chi tiết và triển khai Microservice ALM.
