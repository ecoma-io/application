...
| services/       | Application Service | `<service-name>.service.ts`, `<service-name>.service.test.ts` |
...
- **Triển khai:**
  ...
  - `<command-name>.command.handler.ts`: Xử lý logic orchestration, inject ILogger, repository, domain event publisher, domain service nếu cần. Không chứa logic nghiệp vụ cốt lõi.
...
- **Sử dụng common:** `IGenericResult` (common-application), `common-domain` (domain service, ILogger), `common-types` (Nullable, Maybe, ...).
...
- **Triển khai:**
  ...
  - `<query-name>.query.handler.ts`: Xử lý truy vấn, inject ILogger, repository, mapping sang DTO.
- **Sử dụng common:** `common-types` (Nullable, Maybe), `common-domain` (ILogger).
...
export class GetOrderDetailQueryHandler {
  constructor(
    private readonly orderRepo: IOrderReadRepo,
    private readonly logger: ILogger
  ) {}

  async execute(query: GetOrderDetailQuery): Promise<IGenericResult<any, string>> {
...
### 3.6. Services (Thư mục `services/`)
- **Mục đích:** Chứa các application service phục vụ orchestration/phối hợp nhiều use case hoặc logic điều phối phức tạp.
- **Triển khai:** Mỗi service là một class, có thể inject repository, port, policy, ... Không chứa logic nghiệp vụ cốt lõi, chỉ orchestration.
- **Sử dụng common:** Có thể inject ILogger, Result, ...
- **Unit Test:** Mock các dependency, kiểm tra orchestration đúng.
- **Ví dụ:**
  ```typescript
  // services/order-orchestration.service.ts
  import { IOrderWriteRepo } from '../ports/repository/order.write-repo';
  import { ILogger } from '@ecoma/common-domain';
  ...
  export class OrderOrchestrationService {
    ...
    async cancelOrder(orderId: string): Promise<void> {
      this.logger.debug('Start cancelOrder', { orderId });
      try {
        await this.orderRepo.delete(orderId);
        this.logger.info(`Order ${orderId} cancelled`);
      } catch (err) {
        if (err instanceof DatabaseConnectionError) {
          // Lỗi mất kết nối DB là fatal, không recover được
          this.logger.fatal('Mất kết nối database khi hủy order', err, { orderId });
          throw err;
        }
        this.logger.error('Cancel order failed', err, { orderId });
        throw err;
      }
    }
  }
...
### 3.8. Factories (Thư mục `factories/`)
...
### 3.9. Specifications (Thư mục `specifications/`)
...
### 3.10. Event Handlers (Thư mục `event-handlers/`)
...
### 3.11. Pipelines (Thư mục `pipelines/`)
...
### 3.12. Validators (Thư mục `validators/`)
...
class MustHaveAtLeastOneItemRule implements IValidationRule<CreateOrderCommandDto> {
  getName() { return 'MustHaveAtLeastOneItem'; }
  getDescription() { return 'Đơn hàng phải có ít nhất 1 item'; }
