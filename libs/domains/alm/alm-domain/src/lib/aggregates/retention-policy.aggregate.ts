...
/**
 * Thuộc tính của Retention Policy (đơn giản hóa, không còn rule, chỉ còn điều kiện áp dụng trực tiếp và retentionDays)
 */
...
> {
  ...
  constructor(props: IRetentionPolicyProps) {
    ...
    if (!props.name || props.name.trim().length === 0) {
      throw new InvalidRetentionPolicyError("Policy name must not be empty");
    }
    if (
      !props.retentionDays ||
