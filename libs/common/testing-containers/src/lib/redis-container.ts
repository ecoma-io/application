...
export class StartedRedisContainer extends AbstractStartedContainer {
  constructor(protected override readonly startedTestContainer: StartedTestContainer) {
  ...
  public async executeCliCmd(cmd: string, additionalFlags: string[] = []): Promise<string> {
    ...
    if (result.exitCode !== 0) {
      throw new Error(`executeCliCmd failed with exit code ${result.exitCode} for command: ${cmd}. ${result.output}`);
    }
