import { addQueryParams, getHostname, getQueryParams, getRootDomain, isAbsoluteUrl, joinUrl } from './url-utils';
...
describe('URL Utils', () => {
  ...
  describe('addQueryParams', () => {
    test('Nên thêm query parameters vào URL', () => {
      expect(addQueryParams('https://example.com', { foo: 'bar', baz: 123 })).toBe('https://example.com/?foo=bar&baz=123');
      expect(addQueryParams('https://example.com?a=1', { b: 2 })).toBe('https://example.com/?a=1&b=2');
    });
  ...
  describe('joinUrl', () => {
    ...
    test('Nên xử lý path phức tạp', () => {
      ...
      expect(joinUrl('https://example.com/api', 'v1/users')).toBe('https://example.com/api/v1/users');
    });
  });
});
