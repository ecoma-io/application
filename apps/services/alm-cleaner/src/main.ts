...
import { MicroserviceOptions, Transport, CustomStrategy } from "@nestjs/microservices";
import { NestjsLogger } from "@ecoma/nestjs-logging";
import { AppModule } from "./app.module";
import { CleanupSchedulerHandler } from "./events/handlers/cleanup-scheduler.handler";
import { INestApplication, ValidationPipe } from '@nestjs/common';

async function bootstrap() {
  // Tạo logger với context là "Bootstrap"
  const logger = new NestjsLogger("ALM-Cleaner");
  ...
  try {
    ...
    // Kiểm tra xem CleanupSchedulerHandler đã được đăng ký chưa
    try {
      const cleanupHandler = app.get(CleanupSchedulerHandler);
      logger.debug("CleanupSchedulerHandler được tìm thấy trong container", {
        handlerName: cleanupHandler.constructor.name,
        methods: Object.getOwnPropertyNames(Object.getPrototypeOf(cleanupHandler))
      });

      // Đăng ký message pattern handler trực tiếp - cách cuối cùng để fix
      logger.debug("Đăng ký trực tiếp 'cleanup-audit-logs' pattern vào microservice controller");
      const messagePattern = { cmd: "cleanup-audit-logs" };
      const controller = {
        handleCleanupRequest: cleanupHandler.handleCleanupRequest.bind(cleanupHandler)
      };
      
      // @ts-expect-error - để tránh lỗi typechecking
      microservice.addHandler(
        messagePattern, 
        controller.handleCleanupRequest,
        Transport.NATS,
        true
      );
      
      logger.debug("Đã đăng ký handler cho pattern", { pattern: JSON.stringify(messagePattern) });
    } catch (err) {
      logger.error("Không tìm thấy CleanupSchedulerHandler trong container", err);
    }
    ...
    logger.error("Error starting ALM Cleaner Service", error, {
      timestamp: new Date().toISOString(),
...
process.on("uncaughtException", (error) => {
  ...
  emergencyLogger.fatal("Uncaught exception", error, {
    timestamp: new Date().toISOString(),
    pid: process.pid
  });
  process.exit(1);
...
process.on("unhandledRejection", (reason, promise) => {
  ...
  emergencyLogger.error(
    ...
    reason instanceof Error ? reason : new Error(String(reason)),
    {
      timestamp: new Date().toISOString(),
      pid: process.pid,
      promise: typeof promise === "object" ? JSON.stringify(promise) : String(promise)
    }
  );
});
