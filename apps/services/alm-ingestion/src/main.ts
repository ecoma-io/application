/**
 * @fileoverview Điểm khởi chạy của ALM Ingestion Service
 * @since 1.0.0
 */
...
async function bootstrap() {
  ...
  const logger = new NestjsLogger("ALM-Ingestion");
  
  try {
    // Log thông tin khởi động với timestamp
    const startTime = Date.now();
    logger.info("Starting ALM Ingestion Service", { 
    ...
    app.connectMicroservice<MicroserviceOptions>({
      ...
      options: {
        servers: [natsUrl],
        queue: "alm-ingestion", // Queue group cho load balancing giữa các instance của alm-ingestion
      },
    });
    ...
    logger.info("ALM Ingestion Service started successfully", {
      startupTimeMs: startupTime,
      timestamp: new Date().toISOString(),
      memoryUsage: process.memoryUsage()
    });
  } catch (error) {
    logger.error("Error starting ALM Ingestion Service", error, {
      timestamp: new Date().toISOString(),
