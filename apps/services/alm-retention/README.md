...
```bash
# Build Docker image
nx build alm-retention
docker build -t ecoma/alm-retention:latest -f apps/services/alm-retention/Dockerfile .

# Deploy trên Kubernetes
kubectl apply -f k8s/alm-retention.yaml
```
...
nx serve alm-retention

# Chạy tests
nx test alm-retention
```
