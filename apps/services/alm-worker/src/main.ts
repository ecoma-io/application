...
async function bootstrap() {
  ...
  try {
    ...
    logger.info("Starting ALM Worker Service", {
    ...
    logger.debug("Configuration loaded", {
      ...
      mongodbUri: mongodbUri
        ? `${mongodbUri.split("@")[0].split("//")[0]}//***:***@${
            mongodbUri.split("@")[1]
          }`
        : undefined,
      rabbitmqUri: rabbitmqUri
        ? `${rabbitmqUri.split("@")[0].split("//")[0]}//***:***@${
            rabbitmqUri.split("@")[1]
          }`
        : undefined,
    });
    ...
    app.connectMicroservice<MicroserviceOptions>({
      ...
      options: {
        servers: [natsUrl],
        queue: "alm-worker", // Queue group for load balancing between alm-worker instances
      },
    });
    ...
    logger.info("ALM Worker Service started successfully", {
    ...
    logger.error({
      msg: "Failed to start ALM Worker Service",
      err: error,
    });

    // Ensure process exits with error for container orchestrator to restart
    process.exit(1);
  }
}

// Handle uncaught errors to ensure they are logged
process.on("uncaughtException", (error) => {
  const emergencyLogger = new NestjsLogger("UNCAUGHT_EXCEPTION");
  emergencyLogger.fatal({ msg: "Uncaught exception detected", err: error });
  process.exit(1);
});
...
process.on("unhandledRejection", (reason, promise) => {
  ...
  emergencyLogger.error({
    ...
    promise:
      typeof promise === "object" ? JSON.stringify(promise) : String(promise),
  });
});
